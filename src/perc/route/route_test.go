...
func testParseBackend(t *testing.T, in, en string, ep map[string]string, eerr error) bool {
  ...
  fmt.Printf("%v -> [%v] %v\n", in, b.Addr, b.Params)
  res := true
  res = res && assert.Equal(t, en, b.Addr, "Addrs do not match")
  res = res && assert.Equal(t, ep, b.Params, "Params do not match")
  return res
}

func TestParseRoute(t *testing.T) {
  testParseRoute(t, `:9000=upstream`, &Route{Listen:":9000", Backends:[]Backend{{Addr:"upstream"}}, Service:true}, nil)
  testParseRoute(t, `:9000=host:1234,other:5678`, &Route{Listen:":9000", Backends:[]Backend{{Addr:"host:1234"},{Addr:"other:5678"}}, Service:false}, nil)
  testParseRoute(t, `:9000=host:1234, other:5678`, &Route{Listen:":9000", Backends:[]Backend{{Addr:"host:1234"},{Addr:"other:5678"}}, Service:false}, nil)
  testParseRoute(t, `:9000=upstream(tls='true')`, &Route{Listen:":9000", Backends:[]Backend{{Addr:"upstream", Params:map[string]string{"tls": "true"}}}, Service:true}, nil)
  testParseRoute(t, `:9000 = upstream ( tls = 'true' )`, &Route{Listen:":9000", Backends:[]Backend{{Addr:"upstream", Params:map[string]string{"tls": "true"}}}, Service:true}, nil)
  testParseRoute(t, `:9000=host:1234(tls='true'),other:1234(tls='false')`, &Route{Listen:":9000", Backends:[]Backend{{Addr:"host:1234", Params:map[string]string{"tls": "true"}}, {Addr:"other:1234", Params:map[string]string{"tls": "false"}}}, Service:false}, nil)
