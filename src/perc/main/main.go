...
func main() {
  ...
  fMonitor      := cmdline.String   ("monitor",         oalesce(os.Getenv("HP_API_MONITOR"), ":2222"),                      "The interface and port to accept monitoring (profiling and health check) connections on.")
  fDomain       := cmdline.String   ("domain",          oalesce(os.Getenv("HP_DISCOVERY_DOMAIN"), "disc.hirepurpose.com"),  "The domain to use for service discovery.")
  fDiscovery    := cmdline.String   ("discovery",       oalesce(os.Getenv("HP_DISCOVERY_SERVICE"), "etcd://us-east-1"),     "The discovery service used for service lookup, specified as 'service://[az.]region[,..,[azN.]regionN]'. Regions should be provided in descending order of preference.")
  fInflux       := cmdline.String   ("influxdb",        s.Getenv("HP_METRICS_INFLUXDB"),                                    "The InfluxDB metrics reporting backend, specified as: 'host[:port]'.")
  fEnviron      := cmdline.String   ("environ",         oalesce(os.Getenv("HP_ENVIRON"), os.Getenv("ENVIRON"), "devel"),    "The environment in which the service is running (devel, staging, production).")
  fSentry       := cmdline.String   ("sentry",          s.Getenv("HP_SENTRY"),                                              "Report errors to Sentry. The Sentry authentication DSN should be provided as an argument.")
  fIOTimeout    := cmdline.Duration ("timeout",         trToDur(coalesce(os.Getenv("HP_TIMEOUT"), "0")),                    "Specify both the read and write timeouts for client connections at once. This flag overrides -timeout:read and -timeout:write.")
